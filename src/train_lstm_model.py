...
def train_lstm(model, X_train, y_train, X_val, y_val, learning_rate, epochs, batch_size, model_name):
    model.compile(optimizer='rmsprop', loss='binary_crossentropy', metrics=['accuracy'])
    early_stopping = EarlyStopping(monitor='val_loss', patience=3, restore_best_weights=True)
    checkpoint = ModelCheckpoint(f'results/lstm_results/{model_name}_best_model.h5', monitor='val_loss', save_best_only=True)
    ...
    save_loss_accuracy_curves(history, 'lstm_results', model_name)
    return model
